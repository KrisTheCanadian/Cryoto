describe('Marketplace', () => {
  ...
  it('Test Pagination', () => {
    ...
    cy.get('.MuiPagination-ul > :nth-child(3) > .MuiButtonBase-root').should('exist');
    cy.get('.MuiPagination-ul > :nth-child(3) > .MuiButtonBase-root').click({force: true});
  ...
  it('Test redeeming card', () => {
    cy.visit('/market');
    cy.wait(200);
    cy.get(':nth-child(1) > .MuiPaper-root > .css-32bll7 > [data-testid="cardMediaPic"]').first().click();
    cy.wait(200);
    cy.get('.MuiButton-root').click();
    cy.get('[data-testid="cartButton"] > .MuiBadge-root > .MuiBadge-anchorOriginTopRightRectangular').should('contain', '1');
  });
  ...
  it('Test marketplace filter - Types', () => {
    ...
    cy.get('[data-testid="type-button"]').click();
    cy.wait(100);
    cy.get('[data-testid="CheckBoxOutlineBlankIcon"]').first().click({force: true});
    cy.get('[data-testid="mFilterButton"]').click({force: true});
    cy.wait(1000);
    cy.get(':nth-child(1) > .css-19o132d > .MuiPaper-root > .MuiCardContent-root > .css-0 > .MuiTypography-body1').should('contain', 'Gift Card');
  });
